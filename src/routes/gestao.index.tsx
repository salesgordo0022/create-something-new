        ...
        <main className="flex-1 p-6 lg:p-10">
          ...
          <div className="bg-white rounded-3xl agro-shadow-lg ring-1 ring-gray-100 overflow-hidden">
            <div className="px-6 lg:px-8 py-4 border-b border-gray-100 flex flex-wrap items-center gap-3">
              ...
              <select className="rounded-lg border border-gray-200 bg-gray-50/60 py-1.5 px-3 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#1a5c2a]/20 focus:border-[#1a5c2a] transition-all w-full sm:w-40" value={filtroEstado} onChange={e => setFiltroEstado(e.target.value)}>
                <option value="">Todos os estados</option>
                {estadosDisponiveis.map(e => <option key={e} value={e}>{e}</option>)}
              </select>
              <select className="rounded-lg border border-gray-200 bg-gray-50/60 py-1.5 px-3 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#1a5c2a]/20 focus:border-[#1a5c2a] transition-all w-full sm:w-52" value={filtroStatus} onChange={e => setFiltroStatus(e.target.value)}>
            ...
            <div className="md:hidden divide-y divide-gray-50">
                  ...
                  <div key={f.id} className="px-5 py-4 space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="font-semibold text-gray-900 leading-tight truncate">{f.produtores?.nome_razao}</p>
                        <p className="text-xs text-gray-400 truncate mt-0.5">{f.produtores?.cpf_cnpj || "Documento não informado"}{f.protocolo ? ` • ${f.protocolo}` : ""}</p>
                      </div>
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium ring-1 whitespace-nowrap ${st.badge}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${st.dot}`} />
                        {st.label}
                      </span>
                    </div>
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-gray-500">
                      ...
                      <span className="inline-flex px-2.5 py-0.5 rounded-lg text-xs font-medium bg-[#f2efe8] text-[#5d4037] ring-1 ring-[#5d4037]/10">{f.produtores?.atividade_principal || "—"}</span>
                    ...
                    <div className="flex items-center gap-2">
                      <Link to="/gestao/formulario/$id" params={{ id: f.id }} title="Preencher formulário"
                        className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold text-[#b8942e] bg-[#fef7e6] ring-1 ring-[#e8b830]/20 hover:bg-[#fdf0cf] transition-all">
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                        Preencher
                      </Link>
                      <Link to="/gestao/$id" params={{ id: f.id }}
                        className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold text-white shadow-sm hover:shadow-md transition-all" style={{ background: 'linear-gradient(135deg, #0d4f1a, #1a7a2e)' }}>
                        Visualizar
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                      </Link>
                    </div>
              ...
              {formsFiltrados.length === 0 && (
                <div className="px-5 py-12 text-center text-sm text-gray-400 italic">Nenhum diagnóstico encontrado.</div>
              )}
            </div>
          </div>
        </main>
